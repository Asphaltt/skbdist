...
filter(struct sk_buff *skb)
{
    ...
    void *data = skb_head + skb->mac_header;
    void *data_end = skb_head + skb->tail;
...
set_tuple(struct iphdr *iph, struct net_tuple *tuple)
{
    ...
    case IPPROTO_TCP:
    case IPPROTO_UDP:
        udph = (typeof(udph)) (((void *) iph) + (iph->ihl << 2));
...
{
    void *skb_head = skb->head;
    struct ethhdr *eth = (typeof(eth)) (skb_head + skb->mac_header);
